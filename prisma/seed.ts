...
import { Pool } from 'pg'
import { importCanonicalCourseContent } from '../scripts/import-v2-content'
import {
...
async function main() {
  ...
  const course = buildCanonicalCourseContent(path.resolve(__dirname, '..'))
  assertCanonicalCourseContent(course)
  const content = await importCanonicalCourseContent(prisma, course)

  console.log(
    `Seeded ${content.chapters} chapters, ${content.lessons} lessons and ${content.exercises} exercises`
  )
